...
def search_flights(origin, destination, departure_date, return_date, adults, travel_class, access_token)
  ...
  if return_date.nil?
    ...
    uri = URI("https://test.api.amadeus.com/v2/shopping/flight-offers?originLocationCode=#{origin}&destinationLocationCode=#{destination}&departureDate=#{departure_date_str}&adults=#{adults}&max=5&currencyCode=EUR&travelClass=#{travel_class}")
  else
    return_date_str = return_date.strftime('%Y-%m-%d')
    # Round-trip flight search
    uri = URI("https://test.api.amadeus.com/v2/shopping/flight-offers?originLocationCode=#{origin}&destinationLocationCode=#{destination}&departureDate=#{departure_date_str}&returnDate=#{return_date_str}&adults=#{adults}&max=5&currencyCode=EUR&travelClass=#{travel_class}")
  end
